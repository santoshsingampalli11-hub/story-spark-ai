...
const StoryInspirationComponent: React.FC = () => {
  ...
  // ── Pre-select genre from saved workspace preferences ─────────────────────
  const savedPrefs = getSavedWorkspacePreferences();
  const savedGenreClean = savedPrefs.defaultGenre.replace(/^\p{Emoji}\s*/u, "").trim();
  const initialGenre = genres.includes(savedGenreClean) ? savedGenreClean : "All";
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-28">
        ...
        <motion.div initial={{ opacity: 0, y: -15 }} animate={{ opacity: 1, y: 0 }} className="pt-10 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-3 px-4 py-2 rounded-full bg-white/70 dark:bg-white/[0.04] border border-white/60 dark:border-white/10 backdrop-blur-xl">
            ...
            <span className="text-xs font-semibold tracking-wider uppercase text-slate-700 dark:text-slate-300">AI Powered Inspiration</span>
          </div>
        </motion.div>

        {/* Hero Section */}
        <motion.section initial={{ opacity: 0, y: 35 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="relative pt-20 pb-20 text-center">
          ...
          <div className="inline-flex items-center gap-3 px-5 py-2.5 rounded-full bg-white/80 dark:bg-white/[0.06] border border-white/60 dark:border-white/10 backdrop-blur-xl shadow-lg shadow-black/[0.04] mb-8">
            <div className="relative flex h-3 w-3">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-indigo-400 opacity-75"></span>
              <span className="relative inline-flex rounded-full h-3 w-3 bg-indigo-500"></span>
            </div>
            <span className="text-xs font-bold uppercase tracking-[0.25em] text-indigo-700 dark:text-indigo-300">Creative Story Universe</span>
          </div>
          ...
          <p className="max-w-3xl mx-auto text-lg md:text-xl leading-relaxed text-slate-600 dark:text-slate-400">
            Explore timeless narratives, cinematic worlds, iconic themes, and powerful writing prompts to spark your next masterpiece.
          ...
          <div className="flex flex-wrap justify-center gap-5 mt-12">
            <button
              ...
              className="group px-8 py-4 rounded-2xl bg-gradient-to-r from-indigo-600 to-blue-600 text-white font-bold shadow-2xl shadow-indigo-500/25 hover:scale-[1.03] transition-all duration-300"
            >
            ...
            <button
              ...
              className="px-8 py-4 rounded-2xl bg-white/80 dark:bg-white/[0.05] border-2 border-indigo-400/70 dark:border-indigo-400/50 backdrop-blur-xl font-bold text-indigo-700 dark:text-indigo-300 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 hover:shadow-xl hover:shadow-indigo-500/20 hover:scale-[1.03] transition-all duration-300"
            >
        ...
        {/* Search Section */}
        <motion.section id="discover-section" initial={{ opacity: 0, y: 35 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="mb-20">
          <div className="relative overflow-hidden rounded-[2.5rem] border border-white/60 dark:border-white/10 bg-white/80 dark:bg-white/[0.04] backdrop-blur-2xl shadow-[0_25px_80px_rgba(0,0,0,0.06)] p-6 md:p-10">
            ...
            <div className="relative z-10 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8 mb-10">
              <div>
                <h2 className="text-3xl md:text-4xl font-black text-slate-900 dark:text-white">Discover Inspirations</h2>
              ...
              <div className="px-5 py-4 rounded-2xl bg-gradient-to-r from-indigo-500/10 to-blue-500/10 border border-indigo-500/10 min-w-[220px]">
                <p className="text-sm text-slate-500 dark:text-slate-400">Matching Results</p>
                <h3 className="text-3xl font-black text-indigo-600 dark:text-indigo-400">{filteredStories.length}</h3>
              </div>
            </div>
        ...
        >
          <div className="rounded-[2rem] border border-indigo-200/60 dark:border-indigo-500/20 bg-white/80 dark:bg-white/[0.03] backdrop-blur-xl p-8 md:p-10 shadow-lg">
        ...
        {/* Bottom CTA */}
        <motion.section initial={{ opacity: 0, y: 35 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="mt-8">
          <div className="relative overflow-hidden rounded-[3rem] border border-white/60 dark:border-white/10 bg-gradient-to-br from-indigo-600 via-blue-600 to-purple-700 p-10 md:p-16 text-white shadow-[0_25px_100px_rgba(79,70,229,0.35)]">
            ...
            <div className="relative z-10 max-w-3xl">
              ...
              <h2 className="text-4xl md:text-5xl font-black leading-tight">Ready To Build Your Own Universe?</h2>
              ...
              <div className="flex flex-wrap gap-5 mt-10">
                <button
                  onClick={() => navigate("/stories")}
                  className="group px-8 py-4 rounded-2xl bg-white text-slate-900 font-bold hover:scale-[1.03] transition-all duration-300 shadow-2xl"
                >
                  <span className="flex items-center gap-3">
                    Start Writing
                ...
                <button
                  ...
                  className="px-8 py-4 rounded-2xl border border-white/20 bg-white/10 backdrop-blur-xl font-semibold hover:bg-white/15 transition-all duration-300"
                >
                  Explore More
                </button>
              </div>
